...
class SquadService(BaseService):
    ...
    @classmethod
    async def get_custom_league_leaderboard(cls, custom_league_id: int, tour_id: int) -> List[Dict[str, Any]]:
        async with async_session_maker() as session:
            # Получаем все сквады, которые участвуют в данной кастомной лиге
            stmt = (
                select(custom_league_squads.c.squad_id)
                .where(custom_league_squads.c.custom_league_id == custom_league_id)
            )
            result = await session.execute(stmt)
